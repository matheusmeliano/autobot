...
export function SchedulesClient({
  ...
  function isOverdueByAtLeastOneDay(row: ScheduleRow) {
    try {
      const tz = (String(row.schedule_timezone ?? "").trim() || effectiveTimeZone) as BrazilTimeZone;
      const dueRef =
        String(row.charge_due_at ?? "").trim() ||
        String(row.operational_due_at ?? "").trim() ||
        String(row.data_envio ?? "").trim();
      if (!dueRef) return false;
      const dueLocal = localDateInTimeZone(dueRef, tz);
      const todayLocal = localDateInTimeZone(new Date().toISOString(), tz);
      const [dY, dM, dD] = dueLocal.split("-").map(Number);
      const [tY, tM, tD] = todayLocal.split("-").map(Number);
      const dueDate = Date.UTC(dY, (dM ?? 1) - 1, dD ?? 1, 12, 0, 0);
      const todayDate = Date.UTC(tY, (tM ?? 1) - 1, tD ?? 1, 12, 0, 0);
      const diffDays = Math.floor((todayDate - dueDate) / (24 * 60 * 60 * 1000));
      return diffDays >= 1;
    } catch {
      return false;
    }
      ...
      <div className={variant === "mobile" ? "grid grid-cols-2 gap-2" : "flex flex-nowrap justify-end gap-2"}>
        ...
        <button
          ...
          disabled={
            ...
            (visualStatus.isCurrentMonth && String(r.status ?? "") === "suspeita_de_pagamento") ||
            (visualStatus.isPaid) ||
            (visualStatus.isExecuted && !visualStatus.isPaid && !isOverdueByAtLeastOneDay(r))
          }
  ...
  const triggerNow = (row: ScheduleRow) => {
    ...
    const overdueBool = isOverdueByAtLeastOneDay(row);
    ...
    if (visualStatus.isExecuted && !visualStatus.isPaid) {
      const overdueDays = isOverdueByAtLeastOneDay(row);
      if (!overdueDays) {
