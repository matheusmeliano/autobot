...
export async function getAdminUsersPageData(): Promise<AdminUsersPageData> {
  ...
  if (error) {
    return {
      initial: [],
      errorState: { type: "load", message: error.message },
    };
  }
  ...
  if (expiredSubs.length) {
    try {
      ...
      /* swallow DB errors on background cleanup — keep page working */
    }
  ...
  const initial: AdminUserRow[] = users
    .filter((u) => {
      ...
      const nomeRaw = String(p?.nome ?? (u.user_metadata as any)?.name ?? "")
        .trim()
        .toLowerCase();
