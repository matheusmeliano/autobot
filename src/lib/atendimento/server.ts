...
const EXPERIMENTAL_CLASS_STUDENT_CONFIRMATION_NOTICE_EVENT_TYPE =
  "experimental_class_student_confirmation_whatsapp_sent";
...
export async function maybeSendExperimentalClassConfirmationToStudent(params: {
  ...
  try {
    ...
    const { count: already } = await admin
      ...
      .select("id", { count: "exact", head: true })
      .eq("lead_id", params.leadId)
      .eq("event_type", EXPERIMENTAL_CLASS_STUDENT_CONFIRMATION_NOTICE_EVENT_TYPE);
    if (already && already > 0) return { ok: true, deduped: true };
    ...
    if (!leadRow) return { ok: true, skipped: true, reason: "lead nao encontrado" };

    const leadPhone = String((leadRow as any)?.phone ?? "").trim();
    if (!leadPhone) return { ok: true, skipped: true, reason: "lead sem telefone" };
    ...
    await appendHistoryEvent({
      leadId: params.leadId,
      conversationId: params.conversationId ?? null,
      eventType: EXPERIMENTAL_CLASS_STUDENT_CONFIRMATION_NOTICE_EVENT_TYPE,
      title: "Confirmacao de aula experimental AGENDADA enviada ao aluno (saiu de Falta dia e horario)",
      details: {
        phone: leadPhone,
...
export async function ensureStudentAuthUserCreatedForLead(params: {
  ...
  const assignedUserEmail = String((lead as any)?.assigned_user_email ?? "").trim().toLowerCase();
  if (assignedUserEmail && assignedUserEmail === ATENDIMENTO_EMAIL.toLowerCase()) {
