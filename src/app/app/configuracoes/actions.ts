...
export async function updateTimezoneAction(input: unknown) {
  ...
  const { data: openChargeSchedules, error: schedulesError } = await supabase
    .from("schedules")
    .select(
      "id, status, charge_due_at, data_envio, recurrence_time, payment_received_at, charge:debtor_charges!schedules_charge_id_fkey(due_day, recurrence_month, recurrence_year), debtors(retry_time)",
    )
    .is("closed_at", null)
