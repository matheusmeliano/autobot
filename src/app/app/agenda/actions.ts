...
async function sendZapiText(params: {
  ...
  if (destinationIsInstancePhone(normalizedPhone, null, selfPhoneOpt)) {
    return {
      ok: false,
      skipped: true,
      reason: "self_instance_phone_refused_to_prevent_infinite_loop_at_low_level_agenda_actions_sendZapiText",
  ...
  if (destinationIsInstancePhone(normalizedPhone, runtimeBlocklist, selfPhoneOpt)) {
    return {
      ok: false,
      skipped: true,
      reason: "self_instance_phone_refused_blocklist_matched_whatsapp_instances_agenda",
      phone: normalizePhoneDigitsOnly(normalizedPhone),
    } as any;
  }
...
export async function triggerScheduleNowAction(id: string) {
  ...
  const scheduleTz = String((schedule as any).schedule_timezone ?? "") || "America/Sao_Paulo";
  const nowUtc = new Date().toISOString();
  const todayLocal = localDateInTimeZone(nowUtc, scheduleTz);
  const dueCandidate =
    String((schedule as any).charge_due_at ?? "") ||
    String((schedule as any).operational_due_at ?? "") ||
    String((schedule as any).data_envio ?? "");
  const dueLocal = localDateInTimeZone(dueCandidate || nowUtc, scheduleTz);
  const [dY, dM, dD] = dueLocal.split("-").map(Number);
  const [tY, tM, tD] = todayLocal.split("-").map(Number);
  const dueDateUtc = Date.UTC(dY, (dM ?? 1) - 1, dD ?? 1, 12, 0, 0);
  const todayDateUtc = Date.UTC(tY, (tM ?? 1) - 1, tD ?? 1, 12, 0, 0);
  const diffDays = Math.floor((todayDateUtc - dueDateUtc) / 86400000);
  const isOverdueByAtLeastOneDay = diffDays >= 1;

  const isExecutedNotPaidOverdue =
    loaded.visualStatus.label === "Executado" &&
    !loaded.visualStatus.isPaid &&
    isOverdueByAtLeastOneDay;

  if (loaded.visualStatus.label === "Executado" && !isExecutedNotPaidOverdue) {
  ...
  const blockedStatuses = ["pendente", "suspeita_de_pagamento", "pago"];
  if (currentStatus === "executado" && !isExecutedNotPaidOverdue) {
    blockedStatuses.push("executado");
  }
  if (blockedStatuses.includes(currentStatus)) {
    return { ok: false, error: "Esse agendamento não pode ser reenviado manualmente agora." };
  }

  const lockStatuses = ["agendado", "atrasado", "pausado"];
  if (isExecutedNotPaidOverdue) lockStatuses.push("executado");
  ...
  try {
    ...
    const sourceStatus = isOverdueByAtLeastOneDay || currentStatus === "atrasado" ? "atrasado" : "pendente";
    const timeZone = String((schedule as any).schedule_timezone ?? "") || "America/Sao_Paulo";
    ...
    if (existingRun?.id && !isOverdueByAtLeastOneDay) {
